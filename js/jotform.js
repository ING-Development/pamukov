...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    showField: function (field, multiple) {
        ...
        element.down(".always-hidden") && element.down(".always-hidden").removeClassName('always-hidden');
                    ...
                    setTimeout(function () {
                        ...
                        if (!$$('.form-error-message')[0] && !$$('.form-textarea-limit-indicator-error')[0]) { //Emre: when limit text are is used, submit button doesn't work (51335)
                            ...
                            allButtons.each(function (bu) {
                                if (!(JotForm.payment && JotForm.payment == 'braintree')) { // not for braintree
            ...
            item.observe('blur', function () {
                ...
                if (!event.which || // Control keys in most browsers. e.g. Firefox tab is 0
                    ...
                    if((parseInt(this.value.length) >= parseInt(item.readAttribute('maxlength'))) ||
                        (event.which != 8 && event.which != 0 && event.which != 13 && (event.which < 45 || event.which > 57)) ){
